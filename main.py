...
def grade_date(firstDate, secondDate, set):
    firstYear = set[firstDate][6:10] 
    firstMonth = set[firstDate][3:5]
    firstDay = set[firstDate][0:2]

    secondYear = set[secondDate][6:10]
    secondMonth = set[secondDate][3:5]
    secondDay = set[secondDate][0:2]
...
def print_correct(rightAnswer, wrongAnswer, set):
    rightYear = set[rightAnswer][6:10]
    rightMonth = months[int(set[rightAnswer][3:5])-1]
    rightDay = set[rightAnswer][0:2].lstrip("0")

    wrongYear = set[wrongAnswer][6:10]
    wrongMonth = months[int(set[wrongAnswer][3:5])-1]
    wrongDay = set[wrongAnswer][0:2].lstrip("0")

    print("Correct! " + rightAnswer + " (" + rightMonth + ", " + rightDay + " " + rightYear + ") is older than " + wrongAnswer + " (" + wrongMonth  + ", " + wrongDay + " " + wrongYear + ")")

#evoked when user inputs wrong answer
#first arg is wrong answer (This date is younger)
#second arg is right answer (this date is older)
...
def print_wrong(wrongAnswer, rightAnswer, set):
    ...
    print("Incorrect! " + wrongAnswer + " (" + wrongMonth + ", " + wrongDay + " " + wrongYear + ") is older than " + rightAnswer + " (" + rightMonth + ", " + rightDay + " " + rightYear + ")")
